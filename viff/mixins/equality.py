...
class ProbabilisticEqualityMixin:
    ...
    def equal(self, share_x, share_y):
        ...
        # a = self.prss_share_random(Zp)
        # b = self.prss_share_random(Zp)
        # c = a * a
        # d = a * b
        # e = c * d
        # f = self.open(d * d)
        # f.addCallback(lambda x: 1 / x.sqrt())
        #
        # r = f * e
        # s = f * d
        # z = share_x - share_y
        # z_prime = self.open(((2 * z) + 1) * r)
        # z_prime.addCallback(lambda x: legendre_mod_p(x))
        # u = z_prime * s
        # v = (u + 1) * (1 / Zp(2))
        #
        # return v
